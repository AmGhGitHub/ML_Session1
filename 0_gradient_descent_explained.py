...
def gradient_descent(x, y):
    ...
    while iter_number < max_iter and mse > max_mse:
        ...
        mse = np.sum(np.square(y - y_predict)) / n_data
        parameters_values[iter_number][0] = beta0
        parameters_values[iter_number][1] = beta1
        parameters_values[iter_number][2] = mse
        der_beta1 = -(2 / n_data) * np.sum(x * (y + -y_predict))
        der_beta0 = -(2 / n_data) * np.sum(y - y_predict)
        beta1 -= lr * der_beta1
        beta0 -= lr * der_beta0
        iter_number += 1
...
if __name__ == '__main__':
    ...
    print(f'Iter = {df_m_b_mse.index[-1]}')
    print(f'\u03B20 = {df_m_b_mse.iloc[-1, 0]:.2f}')
    print(f'\u03B21 = {df_m_b_mse.iloc[-1, 1]:.2f}')
